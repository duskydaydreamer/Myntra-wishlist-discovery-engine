...
def test_api():
    ...
    server = subprocess.Popen(["venv/bin/uvicorn", "backend.api.main:app", "--port", "8005"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    time.sleep(3)
    
    try:
        print("Fetching list of opportunities...")
        list_resp = requests.get("http://localhost:8005/api/dashboard/opportunities")
        ...
        for opp in opportunities:
            ...
            detail_resp = requests.get(f"http://localhost:8005/api/opportunities/{opp_id}")
        ...
        nf_resp = requests.get("http://localhost:8005/api/opportunities/non_existent_id")
